...
# XPoint stakable vesting contract script. This script scans a list of deployed stakable
...
parser.add_argument(
    "-X", "--xpnt", help="XPNT token address to validate", metavar="0x..."
)
parser.add_argument(
...
for caddr in args.contracts:
    try:
        ...
        xpnt, bene, revoked, revoker, rewards, contrib, transfer, start, end = (
            x().call()
            for x in (
                c.XPNT,
                c.beneficiary,
        ...
        XPNT = get_contract("XPNT.sol:XPNT", xpnt).functions
        balance = XPNT.balanceOf(c.address).call()
        ...
        results.append(
            [
                c.address,
                f"{balance} {validate(args.xpnt, xpnt)}",
                bene,
                start,
                end,
                revoked,
...
if args.xpnt or args.revoker or args.rewards or args.contrib:
    print(f"\nValidations:")
    if args.xpnt:
        print(f"✅ = XPNT token address {args.xpnt}")
    if args.revoker:
