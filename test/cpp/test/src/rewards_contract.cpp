...
int main(int argc, char *argv[]) {
    ...
    // NOTE: CI exports the address from the fresh local deployment manifest.
    // The config value is only a fallback for manual local runs.
    const char* deployed_rewards_address = std::getenv("SERVICE_NODE_REWARDS_ADDRESS");
    contract_address = deployed_rewards_address && deployed_rewards_address[0] != '\0'
        ? std::string{deployed_rewards_address}
        : std::string{config.REWARDS_CONTRACT_ADDRESS};
