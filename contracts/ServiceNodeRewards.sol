...
contract ServiceNodeRewards is Initializable, Ownable2StepUpgradeable, PausableUpgradeable, IServiceNodeRewards {
    ...
    BLS12381.G1Point private _aggregatePubkey;
    ...
    event ServiceNodeLiquidated(uint64 indexed serviceNodeID, address initiator, BLS12381.G1Point pubkey);
    ...
    error BLSPubkeyDoesNotExist(BLS12381.G1Point pubkey);
    error BLSPubkeyDoesNotMatch(uint64 serviceNodeID, BLS12381.G1Point pubkey);
    ...
    error InvalidBLSSignature(BLS12381.G1Point aggPubkey);
    ...
    function updateRewardsBalance(
        ...
        // NOTE: Validate signature
        {
            ...
            BLS12381.G2Point memory Hm = BLS12381.hashToG2(abi.encodePacked(hashToG2Tag, encodedMessage));
    ...
    /// key, take a staked amount of XPNT tokens, and then add this node to
    ...
    function addBLSPublicKey(
        BLS12381.G1Point memory blsPubkey,
        ...
        uint64 serviceNodeID = serviceNodeIDs[BLS12381.key(blsPubkey)];
        if (serviceNodeID != 0)
    ...
    function _validateProofOfPossession(
        ...
        BLS12381.G2Point memory Hm = BLS12381.hashToG2(abi.encodePacked(hashToG2Tag, encodedMessage));
        BLS12381.G2Point memory signature = BLS12381.G2Point(blsSignature.signature);
        if (!BLS12381.pairing2(BLS12381.generatorG1(), signature, BLS12381.negate(blsPubkey), Hm)) {
    ...
    function exitBLSPublicKeyWithSignature(
        BLS12381.G1Point calldata blsPubkey,
        uint256 timestamp,
        BLSSignatureParams calldata blsSignature,
        uint64[] memory ids
    ) external whenNotPaused whenStarted hasEnoughSigners(ids.length) {
    ...
    function _validateBLSExitWithSignature(
        ...
        bytes memory pubkeyBytes = BLS12381.key(blsPubkey);
        ...
        if (!BLS12381.eq(blsPubkey, node.blsPubkey)) {
        ...
        // NOTE: Validate signature
        {
            bytes memory encodedMessage = abi.encodePacked(signatureTag, blsPubkey.data, timestamp);
            BLS12381.G2Point memory Hm = BLS12381.hashToG2(abi.encodePacked(hashToG2Tag, encodedMessage));
            validateSignatureOrRevert(ids, blsSignature, Hm);
        }
    }
    ...
    function liquidateBLSPublicKeyWithSignature(
        ...
        if (poolShareOfLiquidationRatio > 0) {
            SafeERC20.forceApprove(designatedToken, address(foundationPool), poolAmount);
            IRewardPoolDeposit(address(foundationPool)).deposit(poolAmount);
        }
    }
    ...
    /// `_serviceNodes[id]`).
    function serviceNodeAdd(BLS12381.G1Point memory blsPubkey, uint256 ed25519Pubkey) internal returns (uint64 id, ServiceNode storage sn) {
        // NOTE: Check keys
        BLS12381.requireValidG1(blsPubkey);
        ...
        bytes memory blsPubkeyBytes = BLS12381.key(blsPubkey);
        ...
        if (totalNodes == 1) {
            ...
            _aggregatePubkey = BLS12381.g1Add(_aggregatePubkey, blsPubkey);
        }
    ...
    function serviceNodeDelete(uint64 nodeID) internal {
        ...
        _aggregatePubkey = BLS12381.g1Add(_aggregatePubkey, BLS12381.negate(node.blsPubkey));

        // NOTE: Retrieve node keys
        bytes  memory blsPubkeyBytes = BLS12381.key(node.blsPubkey);
    ...
    /// cycle in atomic XPNT. If the claimed amount over the period of
    ...
    function validateSignatureOrRevert(
        ...
        BLSSignatureParams memory blsSignature,
        BLS12381.G2Point memory hashToVerify
    ) private {
        BLS12381.G1Point memory blsPubkey = BLS12381.zeroG1();
        ...
        for (uint256 i = 0; i < listOfNonSignersLength; ) {
            ...
            blsPubkey = BLS12381.g1Add(blsPubkey, _serviceNodes[serviceNodeID].blsPubkey);
            unchecked { i += 1; }
        }

        blsPubkey = BLS12381.g1Add(_aggregatePubkey, BLS12381.negate(blsPubkey));
        BLS12381.G2Point memory signature = BLS12381.G2Point(blsSignature.signature);

        BLS12381.G1Point memory negPubkey = BLS12381.negate(blsPubkey);
        if (!BLS12381.pairing2(BLS12381.generatorG1(), signature, negPubkey, hashToVerify)) {
    ...
    function aggregatePubkey() external view returns (BLS12381.G1Point memory) {
    ...
    function allServiceNodeIDs() external view returns (uint64[] memory ids, BLS12381.G1Point[] memory pubkeys) {
        ids = new uint64[](totalNodes);
        pubkeys = new BLS12381.G1Point[](totalNodes);
