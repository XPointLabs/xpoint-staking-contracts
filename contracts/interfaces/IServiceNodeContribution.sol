...
interface IServiceNodeContribution {
    ...
    event UpdatePubkeys                 (BLS12381.G1Point newBLSPubkey, uint256 newEd25519Pubkey);
    ...
    function XPNT()                                  external view returns (IERC20);
    function stakingRewardsContract()                external view returns (IServiceNodeRewards);
    function stakingRequirement()                    external view returns (uint256);

    function blsPubkey()                             external view returns (bytes memory data);
    ...
    /// contract or alternatively after they have called `reset`.
    ///
    /// @param newBLSPubkey The new 128 byte BLS12-381 public key for the node.
    /// @param newBLSSig The new 256 byte BLS12-381 proof-of-posession signature that proves
    ...
    /// `operator`. The `amount` of XPNT token must be at least the
    ...
    ///
    /// @param amount The amount of XPNT token to contribute to the contract.
