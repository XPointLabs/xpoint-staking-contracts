...
contract ServiceNodeContribution is Initializable, Shared, IServiceNodeContribution {
    // Definitions
    ...
    // Service Node
    BLS12381.G1Point                              public blsPubkey;
    ...
    /// @notice Initializes a multi-contribution node contract for the
    ...
    function initialize(
    ...
    ) external initializer nzAddr(_stakingRewardsContract) nzUint(_maxContributors) {
        ...
        XPNT                   = IERC20(stakingRewardsContract.designatedToken());
    ...
    function updatePubkeys(BLS12381.G1Point memory newBLSPubkey,
    ...
    function _updatePubkeys(BLS12381.G1Point memory newBLSPubkey,
                            IServiceNodeRewards.BLSSignatureParams memory newBLSSig,
                            uint256 ed25519Pubkey,
                            uint256 ed25519Sig0,
    ...
    function _contributeFunds(address caller, address beneficiary, uint256 amount) private {
        ...
        XPNT.safeTransferFrom(caller, address(this), amount);
    ...
    function _finalize() private {
        ...
        XPNT.approve(address(stakingRewardsContract), stakingRequirement);
    ...
    function _reset() private {
        ...
        for (uint256 i = 0; i < length; ) {
            ...
            if (status != Status.Finalized && refund > 0)
                XPNT.safeTransfer(toRemove, refund);
    ...
    function resetUpdateAndContribute(BLS12381.G1Point memory key,
                                      IServiceNodeRewards.BLSSignatureParams memory sig,
                                      IServiceNodeRewards.ServiceNodeParams memory params,
                                      IServiceNodeRewards.ReservedContributor[] memory reserved,
                                      bool _manualFinalize,
    ...
    function _resetUpdateAndContribute(BLS12381.G1Point memory key,
                                       IServiceNodeRewards.BLSSignatureParams memory sig,
                                       IServiceNodeRewards.ServiceNodeParams memory params,
                                       IServiceNodeRewards.ReservedContributor[] memory reserved,
                                       bool _manualFinalize,
    ...
    ///   4) Refunding the XPNT amount contributed to the contributor
    ///
    /// @return result The amount of XPNT refunded for the given `toRemove`
    ...
    function removeAndRefundContributor(address toRemove) private returns (uint256 result) {
        ...
        if (status == Status.Finalized) {
            ...
            XPNT.safeTransfer(toRemove, result);
