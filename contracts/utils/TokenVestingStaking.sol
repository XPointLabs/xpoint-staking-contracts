...
contract TokenVestingStaking is ITokenVestingStaking, Shared {
    ...
    /// transferred
    /// @param rewardsContract_ The XPNT staking rewards contract that can
    /// be interacted with
    /// @param xpnt_ The XPNT token address.
    constructor(
        ...
        IERC20 xpnt_
    ) nzAddr(beneficiary_) nzAddr(address(rewardsContract_)) nzAddr(address(xpnt_)) {
    ...
    function addBLSPublicKey(
        ...
        XPNT.approve(address(rewardsContract), stakingRequirement);
    ...
    function contributeFunds(address snContribAddr,
        ...
        XPNT.approve(snContribAddr, amount);
